...
"""
    ...
    def post_sns(self, target_date: datetime):
        ...
        # 画像の説明をAIに依頼
        system_prompt = """
        You are an agricultural-vision specialist. 
        Look ONLY at the provided images (latest first) and ignore sensor data such as soil-moisture numbers. 
        Describe what is visually evident.

        Output MUST be valid JSON with this exact structure:

        {
            "focal_change": "<今いちばん目を引く変化を1文>",
            "trend":        "<過去画像と比べた増減・推移を1文>",
            "supporting_details": ["<色・形・姿勢など具体観察×2-3>"],
            "tone":         "<情緒や雰囲気を一語で示す>",
            "creative_choices": {
                "metaphor":  "<大胆な比喩や例えを1文>",
                "haiku":     "<5-7-5の俳句>",
                "personify": "<植物が思っていそうな短いひと言>"
            }
        }

        Rules:
        - Keep each string ≤ 40 Japanese characters.
        - No markdown, no line-breaks inside JSON values.
        ...
        __input_data["image_caption"] = plant_status
        ...
        # ==================================================
        # 土曜日の場合は is_weekly_highlight_day = True
        ...
        # ==================================================
        input_str = ""
        for key, value in __input_data.items():
...
3. Creative Essence (choose **exactly ONE** per post):
   ...
   e. **A culinary flourish** — liken today’s growth to a flavor, aroma.
...
4. Vary phrasing and mood day-to-day so returning followers always find something fresh.
    4-bis. Compare today’s draft with the latest caption supplied in
        "previous_post_summary".
        • Do NOT reuse any sentence or phrase longer than 6 Japanese words.
        • Avoid repeating the same creative essence type two days in a row.
        • Replace or paraphrase any overlap you detect.
...
"""
        ...
        while try_count < 5:
            try_count += 1
            logger.info(f"Generating post content by AI, try {try_count}")
            # AIによる投稿内容の生成
            post_content = self.ai_connector.analyze_text(message, system_prompt=system_prompt)
            if post_content is not None:
                break
            logger.warning(f"Failed to generate post content by AI, retrying...: {try_count}/5")
            time.sleep(try_count * 2)  # Exponential backoff

        if post_content is None:
            logger.error("Failed to generate post content by AI after 3 attempts.")
            return
        ...
        # – generated by an AI assistant 🌱 以降を切る
        post_content = post_content.split("– generated by an AI assistant 🌱")[0].strip()
        # – generated by an AI assistant 🌱 を付与
        post_content += "\n– generated by an AI assistant 🌱"
        logger.info(f"Final post content: {post_content}")
        ...
        # ===================================================
        if __input_data["is_weekly_highlight_day"]:
            # 週次ハイライト日の場合は、過去7日間の画像を取得
            image_bytes_list, video_width, video_height = self._fetch_image_list(camera_id, target_date, fetch_days=7)
        else:
            # 通常の日は、過去3日間の画像を取得
            image_bytes_list, video_width, video_height = self._fetch_image_list(camera_id, target_date, fetch_days=3)
        ...
        if latest_image:
            ...
            # 日付を追加してカバー画像を生成
            cover_text = datetime.now(UTC).strftime("%Y-%m-%d")
            if __input_data["is_weekly_highlight_day"]:
                cover_text += " 週次ハイライト"
            else:
                cover_text += " 成長記録"
    ...
    def _fetch_image_list(self, camera_id: str, target_date: datetime, fetch_days: int = 1):
        """
        Fetch a list of images from the camera for the specified date.
        camera_id: The ID of the camera to fetch images from.
        target_date: The date for which to fetch images.
        fetch_days: The number of days to look back for images.
                    e.g., 2 means fetch images from the target date and the previous day.
        ...
        max_image_len = 6 * 24 * fetch_days  # 6 hours * 24 images per hour * fetch_days
        ...
        for day_offset in range(fetch_days):
            # 日付を取得
            target_date_with_offset = target_date - timedelta(days=day_offset)
            logger.info(f"Fetching images for camera {camera_id} on {target_date_with_offset.strftime('%Y-%m-%d')}")

            # 画像を取得
            date_filter = target_date_with_offset.strftime("%Y%m%d")
            ...
            # 画像をリストに追加
            camera_latest_images.extend(today_images)
        ...
        formated_image_list = []
        for img in camera_latest_images:
            try:
                formated_image_list.append(forming_image(camera_image_repository().download_image(img["key"])))
            except Exception as e:
                logger.error(f"Error processing image {img['key']}: {e}")

        return formated_image_list, video_width, video_height
